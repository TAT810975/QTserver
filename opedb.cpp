...
void OpeDB::init()
{
    m_db.setUserName("localhost");
    m_db.setDatabaseName("E:\\TcpServer\\TcpServer\\cloud.db");
    if(m_db.open())
    {
        QSqlQuery query;
...
int OpeDB::handlSearchUsr(const char *name)
{
    if(NULL == name)
    {
        return -1;
...
void OpeDB::handAddFriendAgree(const char *pername, const char *name)
{
    if(NULL== pername || NULL == name)
    {
        return ;
